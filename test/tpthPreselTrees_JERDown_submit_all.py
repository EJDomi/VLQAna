...
def main():
    ...
    config.General.workArea = '80X_JERDown_jobs_tZ_5Oct16'
    config.General.transferLogs = True

    config.section_("JobType")
    config.JobType.pluginName = 'Analysis'
    config.JobType.psetName = 'vlqana_cfg.py' 
    config.JobType.pyCfgParams = ['maxEvents=-1','isData=False','doPUReweightingOfficial=True','jecShift=0','jerShift=-1', 'HTMin=800','storePreselEvts=True','storeLHEWts=False'] 
    ...
    config.Data.outLFNDirBase = '/store/user/eschmitz/B2G/80X_VLQAna_LHEWts_tZ_5Oct16/JERDown/'
    ...
    for ijob, job in enumerate(jobs) :
        ...
        if (len('JERDown_'+ pd + '_' + processing)<=100): 
          config.General.requestName = 'JERDown_' + pd + '_' + processing
        else:
          config.General.requestName = 'JERDown_' + pd 
