...
def main():
    ...
    config.General.workArea = '80X_JESDown_jobs_tZ_5Oct16'
    config.General.transferLogs = True

    config.section_("JobType")
    config.JobType.pluginName = 'Analysis'
    config.JobType.psetName = 'vlqana_cfg.py' 
    config.JobType.pyCfgParams = ['isData=False','storePreselEvts=True','doPUReweightingOfficial=True','jecShift=-1','jerShift=1', 'HTMin=800','storeLHEWts=False'] 
    ...
    config.Data.outLFNDirBase = '/store/user/eschmitz/B2G/80X_VLQAna_LHEWts_tZ_5Oct16/JESDown'
    ...
    for ijob, job in enumerate(jobs) :
        ...
        if (len('JESDown_'+ pd + '_' + processing)<=100): 
          config.General.requestName = 'JESDown_' + pd + '_' + processing
        else:
          config.General.requestName = 'JESDown_' + pd 
